...
- Сравните, что вернется с фильтром по метаданным и без него: в коде выше уберите `filter={"section": "возврат"}` и посмотрите, как меняется набор top-k.
